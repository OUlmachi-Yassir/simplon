...
<div class="flex items-center justify-center min-h-screen">
    <div class="w-full max-w-md p-6 bg-white rounded-md shadow-md md:max-w-lg lg:max-w-xl">
        ...
        <form action="<?= URLROOT; ?>/categories/editWiki/<?= $wiki->wikiId; ?>" method="post">
            <div class="mb-4">
                <label for="title" class="block text-sm font-semibold text-gray-600">Title:</label>
                <input type="text" id="title" name="title" value="<?= $wiki->title; ?>" required
            ...
            <div class="mb-4">
                ...
                <textarea id="content" name="content" required
                    class="w-full mt-1 p-2 border rounded-md"><?= $wiki->content; ?></textarea>
            </div>
                ...
                <select id="category" name="category" required
                    ...
                    <?php foreach ($data['categories'] as $category) : ?>
                        <option value="<?= $category->categoryId; ?>"
                            <?= ($wiki->categoryId == $category->categoryId) ? 'selected' : ''; ?>>
                            <?= $category->name; ?>
                        </option>
                    <?php endforeach; ?>
                </select>
